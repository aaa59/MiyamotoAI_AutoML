...
def main():
    ...
    if choice == '学習と検証':
        ...
        if 'df' in st.session_state:
            ...
            # セレクトボックスと増減ボタンの表示（決定木の深さ）
            depth = st.sidebar.number_input('決定木の深さを選択してください', min_value=1, max_value=5, value=3)
    ...
    if choice == 'About':
        ...
        image = Image.open('dreamload.png')
        st.image(image)

        st.markdown("Built by [dream_road Team]")
        st.text("Version 0.7.21")
